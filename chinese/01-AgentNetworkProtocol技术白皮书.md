...
为实现敏感操作的人类可控性，ANP 将身份认证与操作授权分离。根据 [ANP-03 v1.1](/chinese/03-did-wba方法规范.md)，`did:wba` DID Document 只声明可用于 `authentication` 的验证方法；它不定义专用的 `humanAuthorization` 验证关系。
...
这种职责分离使身份证明与操作授权保持独立，并允许上层策略对敏感操作提出人类授权要求。ANP-03 不定义如何取得或证明人类授权；这些细节由适用的业务协议和授权策略定义。
