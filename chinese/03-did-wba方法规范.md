...
> ANP 1.2 发布说明：通用 DID 身份认证由 ANP-02 定义；DID 方法与命名规则仍由各自规范拥有。英文镜像为 [did:wba Method Specification](../03-did-wba-method-design-specification.md)。
...
通用 HTTP/JSON 身份认证现由 [ANP-02](02-ANP-基于DID的身份认证协议.md) 定义；本规范提供 WBA 方法规则。原生 Web 的方法绑定、命名和消息组合见[附录 B](附录B：与原生did-web-的兼容.md)。
...
- **service**：可选字段，定义了与DID主体关联的服务列表。
  - **id**：服务的唯一标识符。
  - **type**：服务类型。目前支持以下类型：
    - `AgentDescription`：智能体描述服务，`serviceEndpoint` 指向遵循[ANP-智能体描述协议规范](07-ANP-智能体描述协议规范.md)的文档。
...
本章通用认证见 [ANP-02 第 3 章](02-ANP-基于DID的身份认证协议.md#http-binding)。WBA 方法规则仍由第 2 章定义；请求格式、验证流程和实现策略保持发布前认证设计不变。
...
通用 HTTP/JSON 请求认证和可选 Token 的唯一规范性定义位于 ANP-02。
...
参考文档[附录A：did-wba-k1_兼容扩展.md](附录A：did-wba-k1_兼容扩展.md)
