...
3. 采用 [ANP-02](02-ANP-基于DID的身份认证协议.md) 定义的通用 DID 身份认证，支持 `did:wba` 与原生 `did:web`；DID Document 按对应方法验证
...
智能体描述协议的 DID 请求认证遵循 [ANP-02](02-ANP-基于DID的身份认证协议.md)。`did:wba` 与原生 `did:web` 共用认证流程，身份材料按对应 DID 方法验证；后续方法按 ANP-02 的方法绑定机制扩展。

下列 `DIDWBASecurityScheme` 保留既有 WBA 描述词汇及配置示例，不新增通用 scheme 标识，也不把 WBA 配置直接当作其他 DID 方法的能力声明。接口声明及对端能力应明确其实际支持的方法。
...
- header：参数将在协议提供的头部中给出，头部的名称由name的值提供。ANP-02 首次请求签名使用 `Signature-Input`、`Signature`，有消息体时还使用 `Content-Digest`；`Authorization` 用于后续可选的 Bearer Access Token，不替代首次签名。
...
上述 `Authorization` 配置仅描述可选 Access Token 的承载位置，不能作为 ANP-02 首次签名所需全部头部的声明。具体请求与验证规则以 ANP-02 为准。
...
`humanAuthorization` 用于声明接口级的人类授权要求。它不是 DID 验证关系或授权凭证，也不能证明授权已经完成。基于 DID 的请求认证仍使用 [ANP-02](02-ANP-基于DID的身份认证协议.md) 中由 `authentication` 引用的验证方法。如何取得或证明人类授权不属于该字段的定义范围，应由适用的业务协议和授权策略确定。
