...
- 文档编号：ANP-P6
- 标题：群组端到端加密
- 状态：候选；已合入 1.2 文档，稳定 v2 发布仍待 MLS ExtensionType 注册
- 版本：1.2
...
3. 如何将经方法验证的 Agent DID 及一个合格 `device_id` 与 MLS 成员凭证、KeyPackage、Leaf 签名键进行绑定；
...
- **DID Device Binding**（保留 wire 名称 `did_wba_binding`）：把某个 MLS Leaf 签名键、成员凭证或 KeyPackage 绑定到可验证的 `(agent_did, device_id)` 证明对象。
...
本 Profile 的主线与 DID 身份的关系如下：

- DID 文档中的 `assertionMethod` 用于身份绑定证明；
...
- 叶子签名键 **SHOULD** 单独生成，并通过第 6 节的绑定对象绑定到 `(agent_did, device_id)`。

---

## 6. DID 与 MLS 的方法无关绑定模型
...
本 Profile 定义 `did_wba_binding` 对象，用于把 MLS Leaf 签名键绑定到 `agent_did` 下的一个合格设备。对象名及 `anp_did_wba_device_binding` 扩展名不限制 `agent_did` 的 DID 方法。
...
1. 按 [P2 方法验证](02-身份与发现.md#method-validation)解析并验证 `agent_did` 的当前 DID 文档；
...
2. RFC 9421 签名在被覆盖组件上验签通过，其中 `"@method"` 从重建对象的 `method`（即 `group.e2ee.send`）推导，`"@target-uri"` 按 P1 附录 A.4 取 `anp://group/<pct-encoded body.group_did>`，验证方法为该 proof `keyid` 所引用的、从发送方当前经方法验证的 DID Document 解析出的验证方法；
...
| 5002 | `group.e2ee.did_binding_invalid` | DID/设备与 MLS 绑定验证失败 |
