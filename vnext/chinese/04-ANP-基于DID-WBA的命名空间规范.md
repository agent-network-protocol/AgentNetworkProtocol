...
- 状态：草案 / 未发布
- 已发布基线：[ANP-04 v1.1](../../chinese/04-ANP-基于DID-WBA的命名空间规范.md)
- 语言：中文
- 适用范围：本规范适用于 ANP 中面向人类可读的 Handle 命名、WNS 解析与 did:wba 名称映射。

> 草案说明：本文件是已发布 ANP 1.1 规范的 vNext 初始工作稿。在本草案正式发布前，不得将其视为已发布协议。英文镜像为 [ANP-DID:WBA Name Space Specification](../04-anp-did-wba-name-space-specification.md)。
...
- [端到端即时消息协议规范](../../chinese/09-ANP-端到端即时消息协议规范.md)
