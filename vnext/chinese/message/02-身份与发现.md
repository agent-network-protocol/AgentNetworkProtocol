...
本草案首个注册的自动迁移路径，是最后一段采用 `e1_` 指纹 Profile 的路径型 did:wba DID。解析器 **MUST** 从此前可信的 DID 出发，按 [ANP-03 方法规则](../03-did-wba方法规范.md#wba-method-rules)验证每一跳及最终活动 DID 文档，并拒绝循环、冲突后继和可信缓存冲突。
